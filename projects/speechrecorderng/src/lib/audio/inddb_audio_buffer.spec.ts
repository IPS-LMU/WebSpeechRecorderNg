...
describe('IndexedDbAudioBuffer', () => {
    ...
    //
    //     test1(aab, stNm, or, tt, ti, (err = undefined) => {
    //         if (err) {
    //             done.fail(err);
    //         }
    //         else {
    ...
    it('#framesObs should asynchronously return correct values', async () => {
        ...
        or.onerror = (err) => {
            throw new Error("Could not open indexed database: " + DB_NAME + ": " + err);
        };
