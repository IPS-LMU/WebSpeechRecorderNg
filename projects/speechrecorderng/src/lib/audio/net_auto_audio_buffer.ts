...
export class NetAutoAudioBuffer extends BasicAudioSource implements AudioSource{
  ...
  toString():string{
    ...
    static fromChunkAudioBuffer(aCtx:AudioContext,recordingsService:RecordingService,baseUrl:string,ab: AudioBuffer,frameLen:number,orgFetchChunkFrameLen:number=ab.length):NetAutoAudioBuffer {
      let nab=new NetAutoAudioBuffer(aCtx,recordingsService,baseUrl,ab.numberOfChannels,ab.sampleRate,ab.length,frameLen,null,orgFetchChunkFrameLen);
      nab.ready();
      return nab;
  }

  asyncAudioInputStream(): AsyncFloat32ArrayInputStream | null {
    return new NetAutoAudioInputStream(this);
  }
  ...
  randomAccessAudioStream(): RandomAccessAudioStream {
    return new NetAutoRandomAccessAudioStream(this);
  }
}
...
export class NetAutoRandomAccessAudioStream implements RandomAccessAudioStream{

  private _currCi=0;
  private _ccCache:Float32Array[]|null=null;

  constructor(private _netAb:NetAutoAudioBuffer) {}
...
export class NetAutoAudioInputStream implements AsyncFloat32ArrayInputStream{

  private framePos:number=0;
  private netAbStr:NetAutoRandomAccessAudioStream;
  constructor(private netAb:NetAutoAudioBuffer) {
      this.netAbStr=new NetAutoRandomAccessAudioStream(netAb);
  }
