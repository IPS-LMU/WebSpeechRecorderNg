...
import {AsyncFloat32ArrayInputStream, EditFloat32ArrayInputStream, Float32ArrayInputStream} from "../../io/stream";
...
export class IndexedDbAudioBufferSourceNode extends AudioWorkletNode {
  ...
  // private fillBufferObs():Observable<void>{
  ...
  private fillBuffer(frameOffset?:number) {
    if (this._inddbAudioBuffer && this._audioInputStream && this._aisBufs) {
      let filled = this.filledFrames;
