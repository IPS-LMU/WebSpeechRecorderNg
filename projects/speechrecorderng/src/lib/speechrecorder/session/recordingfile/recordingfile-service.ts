...
import {RecordingFile, RecordingFileDTO} from "../../recording";
...
export class RecordingFileService {
  ...
  constructor(private http: HttpClient,private sprDb:SprDb, @Inject(SPEECHRECORDER_CONFIG) private config?: SpeechRecorderConfig) {
    ...
    let wobs = new Observable<RecordingFile | null>(observer => {
      ...
      rfDescrObs.subscribe(value => {
        ...
        idbObs.subscribe(value => {
          let rfStoreName=RecordingService.KEYNAME
          if(value.objectStoreNames.contains(rfStoreName)){
            let rfTr = value.transaction(rfStoreName)
            let rfSto = rfTr.objectStore(RecordingService.KEYNAME);
            let rfRq=rfSto.get(recordingFileId);

            rfRq.onsuccess = (ev) => {

              let rfDto:RecordingFileDTO=rfRq.result;
              rfDto.audioBlob.arrayBuffer().then((arrB)=> {
                aCtx.decodeAudioData(arrB, (ab) => {
                  let rf = new RecordingFile(rfDto.sessionId, rfDto.itemCode, rfDto.version, ab);
                  rf.recordingFileId = rfDto.uuid;

                  let pi = {itemcode: rfDto.itemCode};
                  rf.recording = pi;

                  observer.next(rf);
                  observer.complete();
                },err => {
                  // Blob to AudioBuffer decoding failed
                  observer.error(err)
                });
              },(err)=>{
                // Blob to ArrayBuffer failed
                observer.error(err)
              });
            }
            rfRq.onerror= (ev)=>{
              // Indexed DB access failed
              observer.error()
            }
          }else{
            // No ind DB store
