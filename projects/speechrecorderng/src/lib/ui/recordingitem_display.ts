...
@Component({
  ...
  template: `
    ...
    <button *ngIf="!screenXs" matTooltip="Toggle detailed audio display"
    ...
    <button matTooltip="Download current recording" *ngIf="enableDownload"
            [disabled]="disableAudioDetails || !audioLoaded"
      ...
      <table style="border-style: none">
        <tr>
          ...
          <td><span matTooltip="Peak level"
                    [style.color]="(peakDbLvl > warnDbLevel)?'red':'black'">{{ peakDbLvl | number:'1.1-1' }} dB </span>
          </td>
        </tr>
        <tr *ngIf="_agc">
          <td>AGC:</td>
...
@Component({
  ...
  template: `
    <div [class]="{audioStatusDisplay:!screenXs,audioStatusDisplayXs:screenXs}">
      <audio-levelbar style="flex:1 0 1%" [streamingMode]="streamingMode"
                      [displayLevelInfos]="_displayLevelInfos"></audio-levelbar>
      <spr-recordingitemcontrols style="flex:0 0 0px" [audioLoaded]="displayAudioBuffer!==null"
                                 [playStartAction]="playStartAction" [playStopAction]="playStopAction"
                                 [peakDbLvl]="peakDbLvl" [agc]="_agc"
                                 (onShowRecordingDetails)="onShowRecordingDetails.emit()"></spr-recordingitemcontrols>
    </div>
  `,
  styles: [`div {
    width: 100%;
